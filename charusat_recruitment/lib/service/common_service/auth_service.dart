...
class AuthenticationService {
  ...
  Future<int> login(String email, String password) async {
    ...
    print("started Login with credential are $email , $password");
    try {
  ...
  Future<bool> addFcmToken(BuildContext context, String fcmToken) async {
    ...
    var body = {
      "student_idno": extractIdFromEmail(email),
      "token": fcmToken,
    };
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        print(
            "Error adding FCM token: ${response.statusCode}, ${response.body}");
        return false;
      }
    } catch (e) {
...
//otp verification
  Future<bool> verifyOtp(String name , String email, String password, String otp) async {
  ...
  Future<bool> regenerateAccessToken(BuildContext context) async {
    ...
    try {
      var response = await http.post(
        Uri.parse(
            '$serverurl/user/token/refresh/'), // Replace with actual endpoint
      ...
      if (response.statusCode == 200) {
        ...
        await _storage.write(
            key: 'access_token', value: responseBody['access']);
        await _storage.write(
            key: 'refresh_token', value: responseBody['refresh']);
        return true;
      } else if (response.statusCode == 401) {
        _redirectToLogin(context);
      }
